...
class Station(Producer):
    ...
    key_schema = avro.load("models/schemas/arrival_key.json")

    #
    # TODO: Define this value schema in `schemas/station_value.json, then uncomment the below
    #
    value_schema = avro.load("models/schemas/arrival_value.json")
    ...
    def __init__(self, station_id, name, color, direction_a=None, direction_b=None):
        ...
        print(f"In Station color: {self.color}")
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        print(f"In station, color: {self.color}, type: {type(self.color)}")
        _avroValue = {
            "station_id": str(self.station_id),
            "train_id": str(train),
            "direction": str(direction),
            "train_status": str(self.__repr__()),
            "prev_station_id": str(prev_station_id),
            "prev_direction": str(prev_direction),
            "line": str(self.color)
        }
        _avroKey = {"timestamp": self.time_millis()}
        print(f"station key: {_avroKey}")
