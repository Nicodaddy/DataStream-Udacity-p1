...
class Weather(Producer):
    ...
    def __init__(self, month):
        ...
        if Weather.key_schema is None:
            with open("models//schemas/weather_key.json") as f:
        ...
        if Weather.value_schema is None:
            with open("models/schemas/weather_value.json") as f:
                Weather.value_schema = json.load(f)

        super().__init__(
            self.weatherTopic, # TODO: Come up with a better topic name
    ...
    def run(self, month):
        ...
        print(_weatherData)
        resp = requests.post(
           f"{Weather.rest_proxy_url}/topics/{self.weatherTopic}",
           headers={"Content-Type": "application/vnd.kafka.avro.v2+json"},
           data=json.dumps(_weatherData)
        )
        try:
